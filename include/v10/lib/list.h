...
#define LIST_INIT(__list, __init_capacity)                                     \
    {                                                                          \
        ...
        int __list_initial_capacity =                                          \
            __list->capacity * sizeof(*__list->data);                          \
        __list->data = malloc(__list_initial_capacity);                        \
        memset(__list->data, 0, __list_initial_capacity);                      \
    }
...
#define LIST_RESIZE(__list)                                                    \
    {                                                                          \
        if (__list->size >= __list->capacity) {                                \
            __list->capacity *= 2;                                             \
            __list->data = realloc(__list->data,                               \
                                   __list->capacity * sizeof(*__list->data));  \
